  ...
  return (
    <div className="glass-card p-4 animate-fade-in-up">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-secondary text-xs font-semibold uppercase tracking-wider">{item.title || '探索代理 · 浏览路径'}</span>
        {item.focus?.map((f) => (
          ...
          <div key={i} className="flex flex-col gap-0.5 text-xs font-mono">
            <div className="flex items-center gap-2">
              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                ...
                step.kind === 'action_start' ? 'bg-warning' : 'bg-text-dim/50'
              }`} />
