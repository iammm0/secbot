...
export type InferenceMode = 'online' | 'local';
...
export function isLocalProvider(provider: Pick<ProviderApiKeyStatus, 'id' | 'group'>): boolean {
  return (provider.group ?? (provider.id === 'ollama' ? 'local' : 'relay')) === 'local';
}

export function filterProvidersByMode(
  list: ProviderApiKeyStatus[],
  mode: InferenceMode,
): ProviderApiKeyStatus[] {
  return list.filter((p) => (isLocalProvider(p) ? 'local' : 'online') === mode);
}
...
/** 分组显示顺序：在线优先，本地置后 */
const GROUP_ORDER: ProviderGroup[] = ['china', 'overseas', 'relay', 'local'];
