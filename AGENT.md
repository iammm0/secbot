...
| 开发 | tsx watch (热重载) |
| CLI 入口 | npm-bin/secbot.js → server/dist/main.js |
| 终端前端 | Ink 4 + React 18 |
| 移动端 | Expo 54 + React Native |
| 桌面端 | Tauri 2 + Vite + React |
...
├── app/                           # Expo 移动端（独立 package.json）
├── desktop/                       # Tauri 桌面端（独立 package.json）
├── scripts/                       # 启动/构建/验证脚本
...
当前对外默认 agent key 是 `secbot-cli`，它在代码里对应的是 `AgentsService` 中的路由逻辑，内部协调：
...
`ToolsService` 在构造时自动扫描 `tools/` 下各子目录的 `index.ts`，收集所有 `BaseTool` 子类实例。每个工具通过继承 `BaseTool` 并实现 `execute()` 方法来注册。
...
2. 继承 `BaseTool`，实现 `name`、`description`、`execute()` 方法
3. 在同目录 `index.ts` 中导出
4. `ToolsService` 会自动发现并注册
