...
class SessionManager:
    ...
    async def handle_message(
        ...
        Args:
            ...
            allowed_tools: 当前模式允许的工具列表（None = 全部允许）
        ...
        # Tool filtering: temporarily restrict tools based on mode
        _tools_backup = None
        if allowed_tools is not None and hasattr(agent_instance, "tools"):
            _tools_backup = list(agent_instance.tools)
            agent_instance.tools = [
                t for t in agent_instance.tools
                if getattr(t, "name", "") in allowed_tools
            ]
            logger.info(
                f"Tool filter active: {len(agent_instance.tools)}/{len(_tools_backup)} tools available"
            )

        # Mode-specific system prompt injection
        _prompt_backup = None
        try:
            from opencode_adapters.system_prompts import build_system_prompt
            mode_key = None
            if force_qa:
                mode_key = "ask"
            elif plan_only:
                mode_key = "plan"
            elif force_agent_flow:
                mode_key = "build"
            if mode_key and hasattr(agent_instance, "system_prompt"):
                _prompt_backup = agent_instance.system_prompt
                agent_instance.update_system_prompt(
                    build_system_prompt(agent_instance.system_prompt, mode_key)
                )
        except ImportError:
            pass
        ...
        # Restore original tools and system prompt after execution
        if _tools_backup is not None:
            agent_instance.tools = _tools_backup
        if _prompt_backup is not None:
            agent_instance.update_system_prompt(_prompt_backup)
    ...
    async def handle_ask_message(self, user_input: str) -> str:
        ...
        3. 记录消息并返回回复
        4. 发射 CONTENT 事件以便 ACP / SSE transport 中继
        ...
        # Emit CONTENT so ACP EventMapper / SSE relay can forward the answer
        await self.event_bus.emit_simple_async(
            EventType.CONTENT, content=response
        )
        await self.event_bus.emit_simple_async(
            EventType.TASK_PHASE, phase="done", detail=""
        )
    ...
    async def delegate_to_subagent(
        ...
        """
        Delegate a task to a sub-agent (explore / general).

        Creates a SubAgentScheduler on-the-fly and runs the task.
        The sub-agent shares the same EventBus so events are visible
        to whatever transport is listening.

        Returns:
            The sub-agent's result text.
        """
        from core.sub_agent import SubAgentScheduler

        scheduler = SubAgentScheduler(
            event_bus=self.event_bus,
            get_agent=self.get_agent,
        )
        task = await scheduler.delegate(
            agent_type=agent_type,
            prompt=prompt,
    ...
    async def delegate_parallel(
        ...
        """
        Delegate multiple tasks in parallel to sub-agents.

        Args:
            tasks: List of (agent_type, prompt[, description]) tuples
            max_parallel: Concurrency limit
            allowed_tools: Shared tool whitelist

        Returns:
            Combined result text from all sub-agents.
        """
        from core.sub_agent import SubAgentScheduler
        ...
        result = await scheduler.delegate_parallel(
            tasks=tasks,
    ...
    async def transition_plan_to_agent(self, plan_text: str) -> str:
        """
        Handle plan_exit: transition from plan mode to agent mode.

        Emits PLAN_APPROVED, constructs a synthetic user message containing
        the approved plan, and executes it via the normal agent flow.

        Args:
            plan_text: The approved plan text from the planning phase.

        Returns:
            The execution result text.
        """
        await self.event_bus.emit_simple_async(
