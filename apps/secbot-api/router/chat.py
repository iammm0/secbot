...
def _event_to_sse(event: Event) -> tuple[str, dict] | None:
    ...
    if t == EventType.PLAN_EXIT_REQUEST:
        return (
            "plan_exit_request",
            {
    ...
    if t == EventType.SUBAGENT_RESULT:
        return (
            "subagent_result",
            {
                "task_id": d.get("task_id", ""),
...
async def _interaction_event_generator(
    ...
    # Mode-based tool filtering (plan mode restricts to read-only tools)
    from opencode_adapters.plan_mode import PLAN_MODE_ALLOWED_TOOLS, ASK_MODE_ALLOWED_TOOLS
    allowed_tools = None
    if request.mode == "plan":
        allowed_tools = list(PLAN_MODE_ALLOWED_TOOLS)
    elif request.mode == "ask":
        allowed_tools = list(ASK_MODE_ALLOWED_TOOLS)
...
async def chat_sync(request: ChatRequest):
    """
    同步聊天接口。按 mode: ask=QA, plan=仅规划（不执行工具）, agent=智能体执行。
    ...
    try:
        ...
        if request.mode == "plan":
            ...
            if hasattr(planner, "plan"):
                response = await planner.plan(request.message)  # type: ignore[attr-defined]
            else:
                response = await planner.process(request.message)  # type: ignore[attr-defined]
            return ChatResponse(response=response, agent="planner")
...
# ---------------------------------------------------------------------------
# Plan exit (plan → agent transition)
# ---------------------------------------------------------------------------

from pydantic import BaseModel as _BaseModel


class PlanExitRequest(_BaseModel):
    session_id: str = ""
    plan_text: str = ""


@router.post("/plan-exit", summary="批准计划并切换到执行模式")
async def plan_exit(body: PlanExitRequest):
    """
    Approve the pending plan and transition from plan mode to agent mode.
    Follows the opencode PlanExitTool pattern.
    """
    ...
    controller.set_mode(sid, SessionMode.PLAN)
    result = await controller.exit_plan(sid, body.plan_text)
