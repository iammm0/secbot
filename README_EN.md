...
### Browser subprocess tools (optional)

The repository includes a dedicated JS subprocess package for browser automation:
`packages/secbot-browser-tools`.
...
Optional override (if you want Python to use a custom launch command):
...
If you use browser workflows, install `agent-browser` globally:
...
If `agent-browser install` fails on restricted networks, use local Chrome directly:

```powershell
$env:AGENT_BROWSER_EXECUTABLE_PATH = "C:\Program Files\Google\Chrome\Application\chrome.exe"
```

### Multiple browser providers (new)

`packages/secbot-browser-tools` now supports multiple providers:

- `agent-browser` (default, good for existing agent-browser workflows)
- `playwright` (no agent-browser required, runs directly with Playwright)

Switch provider with environment variable:

```powershell
$env:SECBOT_BROWSER_PROVIDER = "playwright"
```

Or explicitly use agent-browser:
