...
export class ContainerEscapeCheckTool extends BaseTool {
  ...
  async run(_params: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (privileged) {
      findings.push({ vector: 'privileged_mode', severity: 'critical', detail: '容器以 --privileged 运行，可直接挂载宿主机文件系统逃逸' });
    }

    // Check Docker socket mount
    const dockerSock = await this.fileExists('/var/run/docker.sock');
    if (dockerSock) {
      findings.push({ vector: 'docker_socket', severity: 'critical', detail: 'Docker socket 已挂载，可通过 API 创建特权容器逃逸' });
    }
    ...
    if (foundCaps.length) {
      findings.push({ vector: 'dangerous_capabilities', severity: 'high', detail: `危险 capabilities: ${foundCaps.join(', ')}` });
    }
    ...
    for (const p of sensitivePaths) {
      if (await this.fileExists(p)) {
        findings.push({ vector: 'sensitive_mount', severity: 'high', detail: `敏感路径可访问: ${p}` });
        break;
      }
    }
    ...
    if (cgroupWritable) {
      findings.push({ vector: 'writable_cgroup', severity: 'high', detail: 'cgroup 可写，可能通过 release_agent 逃逸' });
    }

    // Check service account token (K8s)
    const saToken = await this.fileExists('/var/run/secrets/kubernetes.io/serviceaccount/token');
    if (saToken) {
      findings.push({ vector: 'k8s_service_account', severity: 'medium', detail: 'K8s ServiceAccount token 存在，检查 RBAC 权限' });
    }

    // Check host PID/network namespace
    const hostPid = await this.checkHostPidNs();
    if (hostPid) {
      findings.push({ vector: 'host_pid_namespace', severity: 'high', detail: '共享宿主机 PID namespace，可访问宿主机进程' });
    }
  ...
  private async isInContainer(): Promise<boolean> {
    return (await this.fileExists('/.dockerenv')) ||
